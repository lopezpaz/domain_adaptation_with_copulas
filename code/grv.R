...
gcopula_fit <- function(d)
{
  gcop <- normalCopula(fitCopula(normalCopula(0.5), d)@estimate)
  list(pdf=function(u0,v0) dCopula(cbind(u0,v0),gcop))
}
...
grv <- function(d, unlabeled=NULL)
{
  ...
  for(i in cparams)
  {
    if(n == i[2])
    {
      copulas[i[1],i[2]] <- list(gcopula_fit(cbind(d[,i[1]], d[,n])))
    }
    
    else
    {
      copulas[i[1],i[2]] <- list(gcopula_fit(cbind(X[,i[1]], X[,i[2]])))
    }
  }
